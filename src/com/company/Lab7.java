...
/**
 * CPSC 424, Fall 2015, Lab 7: Image Textures in OpenGL/JOGL. When run as an
 * application, the program shows a window with two panels: on the left, a
 * PaintPanel where the user can draw 2D images and on the right a GLJPanel that
 * shows a single object to which a texture can be applied. There is a menu for
 * selecting the object and one for selecting the texture. It is possible to
 * apply the image from the paint panel as a texture on the 3D object, and it
 * possible to copy the image from the OpenGL panel to the paint panel. This
 * program depends on PaintPanel.java, TexturedShapes.java, Camera.java, and
 * three resource files (brick.jpg, earth.jpg, and clouds.jpg) that contain
...
public class Lab7 extends JPanel implements GLEventListener {
    ...
    /**
     * Constructor for class Lab7, sets the preferred size and configures event
     * listeners
     */
    ...
    /**
     * Create a JOGL Texture from an image that is given as a resource file in the
     * program.
     *
    ...
    /**
     * Copy the image from the OpenGL display panel to the PaintPanel. (Note that
     * this method is called from the event-handling thread. To work with the OpenGL
     * context, that context must be made current on the event-handling thread (if
     * it is not already -- and it presumably won't be). If the context has to be
     * made current, it is important to release it.)
     */
    ...
    /**
     * The display method is called when the panel needs to be drawn. Here, it draws
     * a stage and some objects on the stage.
     */
     ...
     * constant for the rest of the program. It also sets the background color to
     * white.
     */
    ...
    /**
     * Creates a menu bar for use in this program. The menu bar consists of a menu
     * bar from the PaintPanel, with two additional menus. The menu bar from
     * PaintPanel contains commands relevant to drawing on that panel. The two
     * additional menus let the user select the displayed 3D object and texture.
     */
    ...
    public JMenuBar getMenuBar() {
        ...
        ActionListener textureListener = e -> {
            ...
            switch (itemNum) {
                ...
                case 3:
                    try {
                        currentTexture = textureFromResource("com/company/earth.jpg");
                ...
                case 4:
                    try {
                        currentTexture = textureFromResource("com/company/brick.jpg");
                ...
                case 5:
                    try {
                        currentTexture = textureFromResource("com/company/clouds.jpg");
